...
public class ChartOfAccountsDao {
	 ...
	 //for updating
	 public void update(long accountsId, String chartDescription,
	      String chartType) {
	     emf=databaseHelper.provideFactory();
	  em=emf.createEntityManager();
	  em.getTransaction().begin();
	 ...
	 //for displaying
	  public List<ChartOfAccounts>listChartOfAccounts(){
	        emf=databaseHelper.provideFactory();
	        em=emf.createEntityManager();
	        TypedQuery<ChartOfAccounts> query=em.createQuery("Select b from ChartOfAccounts a",ChartOfAccounts.class);
